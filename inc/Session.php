...
class Session
{
  ...
  public static function create_session(bool $one_time): array
  {
    ...
    $accessTokenExpiry =
      $one_time === true
        ? date("Y-m-d h:i:s", $currentTime + 24 * 60 * 60)
        : date(
          "Y-m-d h:i:s",
          $currentTime + self::$_expire_access_token_after * 24 * 60 * 60
        );
    ...
    $refreshTokenExpiry =
      $one_time === true
        ...
        : date(
          "Y-m-d h:i:s",
          $currentTime + self::$_expire_refresh_token_after * 24 * 60 * 60
        );
  ...
  public static function verify_session(): array
  {
    ...
    if (
      ...
      !isset($cookieData->refresh_token)
    ) {
      send_response(false, 401, ["you must be a logged in user to continue"]);
    }
     ...
     */
    if (
      !self::is_valid_base64($cookieData->access_token) ||
      !self::is_valid_base64($cookieData->refresh_token)
    ) {
      send_response(false, 401, ["you must be a logged in user to continue"]);
    }
   ...
   * @return void
   */
  public static function remove_session(): void
  {
    self::verify_session();
    $cookieData = json_decode($_COOKIE[self::$_cookie_name]);

    Query::delete(self::$_table_name, [
      "access_token" => $cookieData->access_token,
      "refresh_token" => $cookieData->refresh_token,
    ]);
    self::remove_cookie();
    send_response(true, 204, ["logout successful"]);
  }
  ...
  {
    $expireTime =
      isset($sessionData["one_time"]) && $sessionData["one_time"] === true
        ? 0
   ...
   * Remove cookie
   */
  private static function remove_cookie(): void
  {
    /**
     * One month before
     */
