...
class Media extends CustomException
{
  ...
  public function handle_image(string $prefix = ""): string
  {
    ...
    if (
      !is_array($info) ||
      !in_array(str_replace("image/", "", $info["mime"]), $this->_valid_image)
    ) {
      throw new CustomException(
        "invalid image! accepted format are " . join(", ", $this->_valid_image)
      );
    }

    if ($this->_file["size"] > $this->_maxImageUpload) {
      throw new CustomException(
        "file size is too large! maximum image upload size is " .
