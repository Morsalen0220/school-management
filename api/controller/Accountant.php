...
class Accountant
{
  private static $_table_name = "accountant";
  ...
  public static function create(): void
  {
    ...
    if (
      ...
      strlen(trim($data->password)) < 10
    ) {
      $errorMessages[] =
        "password is required and length must be at least 10 character";
    }
    ...
    if (count($prevUser) !== 0) {
      send_response(false, 400, [
        "'$data->email' is already in use, try to login or use another email",
      ]);
    }
    ...
    $dbData = [
      ...
      "created_by" => LOGGEDIN_IN_USER_ID,
    ];

    Query::insert(self::$_table_name, $dbData);
  ...
  public static function get_specific(string $userId): void
  {
    ...
    $data = Query::get_specific(
      self::$_table_name,
      ["id", "user_id", "name", "email", "image", "created_by", "created_at"],
      [
        "user_id" => $userId,
      ],
      true,
  ...
  public static function get(): void
  {
    ...
    send_response(
      true,
      200,
      ["accountant list returned successfully"],
      $data,
      true
    );
  }
  ...
  public static function update(string $userId): void
  {
    ...
    if (isset($data->name)) {
      ...
      preg_match('/[A-z .]+$/i', $name) !== 0
        ? ($allowedField["name"] = $name)
    ...
    if (isset($data->image)) {
      ...
      empty($image)
        ? ($errorMessages[] = "invalid image")
        : ($allowedField["image"] = $image);
    }
    ...
    $dbData = Query::update(self::$_table_name, $allowedField, [
      "user_id" => $userId,
    ]);
  ...
  public static function delete(string $userId): void
  {
    ...
    if (LOGGEDIN_IN_USER_ID !== $userId) {
      send_response(false, 401, [
        'sorry, you are not allowed to change someone else\'s info',
      ]);
    }
    ...
    $isDeleted = Query::delete(self::$_table_name, [
      "user_id" => $userId,
    ]);

    if (!$isDeleted) {
      send_response(true, 500, [
        "something went wrong please try again later!",
      ]);
    }

    send_response(true, 204, ["user deleted successfully"]);
  }
}
