...
class School
{
  ...
  public static function create(): void
  {
    ...
    if (
      ...
      !empty(trim($data->description) && strlen(trim($data->description)) > 499)
    ) {
      $errorMessages[] = "description must be less than 500 character";
    }
    ...
    if (!isset($data->school_id) || empty(trim($data->school_id))) {
      ...
      if (count($school) !== 0) {
        send_response(false, 400, [
          "school id already exists, please use another id or keep empty for generating auto id",
        ]);
      }
    }
    ...
    if (isset($data->description) && !empty(trim($data->description))) {
      $PreData["description"] = filter_var(
    ...
    Query::insert(self::$_table_name, $PreData);
  ...
  public static function update(string $school_id): void
  {
    ...
    if (isset($data->description)) {
      $columns["description"] = filter_var(
        trim($data->description),
        FILTER_SANITIZE_STRING
      );
      empty(trim($data->description)) || strlen(trim($data->description)) > 499
    ...
    if (isset($data->school_id) && !empty(trim($data->school_id))) {
      ...
      if (count($school) !== 0) {
        send_response(false, 400, [
          "id already exists, please use another id or keep empty for generating auto id",
        ]);
      }
    }
     ...
     * Add to database
     */
    $DBdata = Query::update(self::$_table_name, $columns, [
      "school_id" => $school_id,
    ]);
